...
// that strikes many targets, the total number of parry ROLLS across all
// targets is capped at 5. Applied via checkParryOfficial in GUILD-TRIAL mode
// only (OFFICIAL_PARRY_GATE) — zone/labyrinth keep the legacy checkParry model.
const MAX_PARRY_ATTEMPTS = 5;

// NOTE: The old labyrinth "maze" player-buff mechanism (MAZE_DEFAULTS /
// resolveMazeBonuses / Player.applyMazeBonuses) was removed. Those hardcoded
// bonuses (+15 combat levels, +15% attack speed, +6% regen/crit, +10% crit
// damage) merely duplicated the labyrinth crate buffs — the Expert Coffee
// Crate grants the levels/atk-speed/crit/crit-damage and the Expert Food
// Crate grants the regen. Applying them again via `options.maze` double-
// counted the crates and badly inflated win rates. Crates (carried on
// `player.zoneBuffs` via Labyrinth.buffs) are now the single source of truth.
...
class CombatSimulator extends EventTarget {
    constructor(players, zone, labyrinth, options = {}) {
        ...
        // Trial run terminates the moment the party wipes (no retry of a tier).
    ...
    async simulate(simulationTimeLimit) {
        ...
        if (this.guildTrial) {
            const cap = GuildTrial.TRIAL_DURATION_NS;
            simulationTimeLimit = simulationTimeLimit ? Math.min(simulationTimeLimit, cap) : cap;
        }

        let ticks = 0;
        // Defence in depth: cap total event-loop iterations per simulation
        // so a future wiring bug surfaces as a thrown error rather than
        // a frozen browser tab. At a sensible 1 ms attack interval, a
        // 120 s sim is ~120 000 events, so 5 M leaves a safety factor
        // of ~40×.
        ...
        while (this.simulationTime < simulationTimeLimit && !this.trialEnded) {
            ...
            if (totalTicks >= MAX_TICKS) {
                throw new Error(
                    `csim simulate(): exceeded MAX_TICKS=${MAX_TICKS} at simTime=${(this.simulationTime / 1e9).toFixed(2)}s ` +
                    `(limit=${(simulationTimeLimit / 1e9).toFixed(0)}s) — pathological event rate, likely a wiring bug`
                );
            }
            if (ticks == 1000) {
                ticks = 0;
        ...
        for (let i = 0; i < this.players.length; i++) {
            ...
            // bookkeeping to avoid reporting bogus drop stats (setManaUsed stays,
            // it is loot-independent and useful).
            if (!this.guildTrial) {
        ...
        if (this.guildTrial) {
            // A wipe or cap-completion already recorded its own endReason (and
            // final-tier HP progress) on the simResult; the "timeout" fallback
            // here covers the only other exit: the loop hitting the 1-hour
            // simulated-time cap. Timeout HP progress:
            //   - mid-encounter (enemies live): fraction of the encounter's
            //     total max HP removed so far;
            //   - between encounters (3 s respawn gap, enemies === null): the
            //     PREVIOUS tier was just cleared and the ladder already
            //     advanced, so finalTier names the NEXT, never-spawned
            //     encounter — truthfully 0 progress on it (the cleared tier is
            //     fully accounted in maxTierCleared / tierTimes).
            this.simResult.finalizeGuildTrial(this.guildTrial, {
                endReason: "timeout",
                endTime: this.simulationTime,
                hpRemovedFrac: this.enemies ? this._trialEncounterHpRemovedFrac() : 0,
            });
        }
    ...
    reset() {
        ...
        // Guild Trial run state.
        this.trialEnded = false;
        this.trialTierStartTime = 0;
    ...
    async processEvent(event) {
        ...
        switch (event.type) {
            ...
            case EnemyRespawnEvent.type:
                ...
                break;
            case LabyrinthTimeoutEvent.type:
                this.processLabyrinthTimeoutEvent(event);
                break;
    ...
    processLabyrinthTimeoutEvent(event) {
        // The room's 120 s clock expired with the monster still standing.
        //
        // Stale-event guards: a kill or wipe already clears the event queue
        // (removing a fired room's pending timeout), so under normal flow this
        // only fires for a genuinely-unresolved room. The two checks below
        // reject any timeout that has outlived its encounter — if the enemies
        // were already cleared, or the active encounter's start time no longer
        // matches the one this event was scheduled for.
        if (!this.labyrinth) return;
        if (!this.enemies) return;
        if (event.encounterStartTime !== this.labyrinth.enconterStartTime) return;

        // FAILED room: deliberately do NOT call addEncounterEnd() and do NOT
        // grant experience — the monster survived the timer. Log the outcome
        // with the monster's surviving HP% first, then reset and start the
        // next attempt at the same instant, mirroring the kill/wipe restart
        // path in checkEncounterEnd().
        if (this.enemies[0]) {
            let mob = this.enemies[0].combatDetails;
            let pct = mob.maxHitpoints > 0 ? (mob.currentHitpoints / mob.maxHitpoints) * 100 : 0;
            this.simResult.addLabRoomOutcome("timeout", Math.max(0, pct), this.simulationTime, event.encounterStartTime);
        }
        this.enemies = null;
        this.eventQueue.clear();
    ...
    startNewEncounter() {
        ...
        if (this.guildTrial) {
            // Spawn the current tier's encounter. The tier was already advanced
            // on the previous clear (see checkEncounterEnd), so this fetches the
            // NEXT tier's monsters. No per-encounter timer (unlike labyrinth).
            this.enemies = this.guildTrial.getEncounter();
            this.trialTierStartTime = this.simulationTime;
            this.simResult.recordTrialTierStart(this.guildTrial.currentTier, this.simulationTime);
        }

        if (this.labyrinth) {
            this.enemies = this.labyrinth.getMonster();
            this.labyrinth.updateEnconterStartTime(this.simulationTime);
            // Hard 120 s cutoff. Scheduled at exactly start + ROOM_DURATION_NS,
            // this event is popped from the time-ordered queue before any attack
            // scheduled past the ceiling — so a killing blow that would have
            // landed after the timer expired is never simulated, and the room is
            // correctly recorded as a FAILED attempt rather than a clear.
            let labyrinthTimeoutEvent = new LabyrinthTimeoutEvent(
                this.simulationTime + Labyrinth.ROOM_DURATION_NS,
                this.simulationTime
            );
            this.eventQueue.addEvent(labyrinthTimeoutEvent);
        }
        ...
        // MWIX adaptation: snapshot the actual player & monster combat stats
        // for the labyrinth "Lab Stats" UI panel. By here players have been
        // reset with their permanent buffs (processCombatStartEvent) and the
        // monster was just reset above, so combatDetails holds exactly the
        // values the simulation will use. captureStatSnapshot is idempotent,
        // so this records only the first labyrinth encounter.
        if (this.labyrinth) {
            this.simResult.captureStatSnapshot(this.players, this.enemies);
        }

        // No enrage in guild trials: trial monsters carry enrageTime 600s in
        // data, but the mode has no enrage mechanic — never schedule the tick.
    ...
    // LEGACY parry model (zone / labyrinth): one roll by a RANDOM parry-capable
    // unit among the defenders; on success the attack is redirected to the
    // attacker (and, in the ability path, the whole cast breaks). Preserved
    // verbatim for zone/lab parity — see OFFICIAL_PARRY_GATE.
    checkParry(targets) {
    ...
    // OFFICIAL_PARRY_GATE — official parry model (guild trials).
    // Official rule: "Each incoming attack can receive at most 5 parry
    // attempts." The STRUCK target itself rolls at its own parry chance; every
    // roll (success or failure) consumes one of the attack event's
    // MAX_PARRY_ATTEMPTS attempts; a target without a parry stat consumes no
    // attempt and gets no roll. `parryState` ({ attempts }) is shared across
    // all instances of one attack event (one swing incl. its pierce/mayhem
    // chain, or one ability cast incl. all AoE targets).
    // Returns true iff this target parries this damage instance.
    checkParryOfficial(target, parryState) {
        if (!target || target.combatDetails.currentHitpoints <= 0) {
            return false;
        }
        if (!(target.combatDetails.combatStats.parry > 0)) {
            return false; // no parry stat — consumes no attempt
        }
        if (parryState.attempts >= MAX_PARRY_ATTEMPTS) {
            return false; // attempt budget for this attack exhausted
        }
        parryState.attempts++;
        return target.combatDetails.combatStats.parry > Math.random();
    }
    ...
    processAutoAttackEvent(event) {
        ...
        // OFFICIAL_PARRY_GATE (see checkParryOfficial / processAbilityDamageEffect
        // for the full description). Guild trials: the struck target rolls its
        // OWN parry, ≤ MAX_PARRY_ATTEMPTS attempts across this swing's
        // pierce/mayhem chain. Zone/labyrinth: legacy model, byte-for-byte —
        // a random parry-capable defender rolls on every hit; success redirects
        // the swing. To make the official rule universal later, delete the
        // legacy branch below (and its twin in processAbilityDamageEffect).
        const useOfficialParry = !!this.guildTrial;
        const parryState = { attempts: 0 };
        ...
        for (let i = 0; i < aliveTargets.length; i++) {
            ...
            if (useOfficialParry) {
                // Official: the target actually being struck rolls its own parry.
                if (this.checkParryOfficial(target, parryState)) {
                    parryTarget = target;
                }
            } else {
    ...
    // Guild Trial debugging: fraction (0..1) of the CURRENT encounter's total
    // max HP (summed across all its monsters — tier, participant and enemyScale
    // scaling already baked into maxHitpoints) that has been removed so far.
    // Lets a run report "how close" the party got to a tier it failed.
    _trialEncounterHpRemovedFrac() {
        ...
        for (const enemy of this.enemies) {
            totalMax += enemy.combatDetails.maxHitpoints;
            totalCurrent += Math.max(0, enemy.combatDetails.currentHitpoints);
        }
        if (totalMax <= 0) {
            return 0;
        }
        return Math.min(1, Math.max(0, (totalMax - totalCurrent) / totalMax));
    }
    ...
    checkEncounterEnd() {
        ...
        // Tier a death/wipe should be attributed to = the tier being fought when
        // this check began. Captured before the clear block below advances the
        // ladder, so a player who dies in the SAME check that clears the tier is
        // still recorded against the tier they actually died in.
        const trialTierAtCheck = this.guildTrial ? this.guildTrial.currentTier : 0;
        ...
        if (this.enemies && !this.enemies.some((enemy) => enemy.combatDetails.currentHitpoints > 0)) {
            ...
            // Trial combat grants NO experience — skip the XP calc/grant entirely.
            if (!this.guildTrial) {
            ...
            if (this.labyrinth) {
                // Cleared the room: monster is dead, so 0% HP remaining.
                this.simResult.addLabRoomOutcome("win", 0, this.simulationTime, this.labyrinth.enconterStartTime);
            }
            if (this.guildTrial) {
                // Tier cleared. Record clear time for the tier just beaten, then
                // step the ladder so the EnemyRespawnEvent (3 s) spawns the NEXT
                // tier. Players are NOT reset between tiers — HP/MP, buffs and
                // cooldowns carry over (dungeon-wave-like continuation).
                const timeSpent = this.simulationTime - this.trialTierStartTime;
                const clearedCapTier = this.guildTrial.currentTier >= GuildTrial.MAX_TIER;
                this.simResult.recordTrialTierClear(this.guildTrial.currentTier, timeSpent, this.simulationTime);
                this.guildTrial.advanceTier();
                if (clearedCapTier) {
                    // Cap tier (300) cleared — nothing further to fight. The
                    // trial is a single weekly attempt with NO re-clear of any
                    // tier, so the run COMPLETES here (endReason "completed",
                    // distinct from "wipe"/"timeout"). Clearing the queue also
                    // removes the EnemyRespawnEvent scheduled above, so no
                    // further encounter spawns; simulate()'s loop then exits
                    // on this.trialEnded.
                    this.simResult.recordTrialCompleted(this.simulationTime);
                    this.trialEnded = true;
                    this.eventQueue.clear();
                }
            }
            this.simResult.lastEncounterFinishTime = this.simulationTime;
            // MWIX adaptation: record FIRST kill time so labyrinth callers
            // can report a single-attempt clear time without averaging
            // across continuous-fight cycles.
            if (this.simResult.firstEncounterFinishTime === 0) {
                this.simResult.firstEncounterFinishTime = this.simulationTime;
            }
        ...
        this.players.forEach(player => {
            if (this.guildTrial) {
                // Trial: dead players STAY DEAD (no PlayerRespawnEvent ever). The
                // zone/lab re-count guard is a pending respawn event, which never
                // exists here, so gate on a per-run "already counted dead" set to
                // avoid re-recording the same death on every checkEncounterEnd.
                // The set entry is cleared by processAbilityReviveEffect so a
                // revive→re-death counts as a second death.
                if (player.combatDetails.currentHitpoints <= 0 && !this.trialDeadPlayers.has(player.hrid)) {
                    this.trialDeadPlayers.add(player.hrid);
                    this.simResult.addDeath(player);
                    this.simResult.recordTrialPlayerDeath(player.hrid, trialTierAtCheck, this.simulationTime);
            ...
            if ((player.combatDetails.currentHitpoints <= 0) && !this.eventQueue.containsEventOfTypeAndHrid(PlayerRespawnEvent.type, player.hrid)) {
                ...
                // MWIX adaptation: csim records monster deaths via
                // addDeath() but skips it for players. Callers that read
                // simResult.deaths to compute win/loss (labyrinth attempt
                // success rate, dungeon attempt rate, single-trial zone
                // win rate) had no signal. Record player death here.
                // Safe against double-counting: the !contains check above
                // gates zone re-entry until respawn fires; for labyrinth
                // the event queue is cleared at L799 and the player is
                // reset by the next CombatStartEvent, so checkEncounterEnd
                // cannot re-observe HP <= 0 for the same player.
                this.simResult.addDeath(player);
                this.simResult.addRanOutOfManaCount(player, false, this.simulationTime);
        ...
        if (
            ...
            if (this.guildTrial) {
                ...
                // encounter's total HP the party had removed), clear the queue
                // and set the flag so simulate()'s loop exits. maxTier reached
                // is derived from the ladder state in finalizeGuildTrial().
                // enemies is null here only when this same check ALSO cleared
                // the encounter (mutual kill via thorns/retaliation) — the
                // encounter was then fully removed: 1.0.
                const hpRemovedFrac = this.enemies ? this._trialEncounterHpRemovedFrac() : 1;
                this.simResult.recordTrialWipe(trialTierAtCheck, this.simulationTime, hpRemovedFrac);
                this.trialEnded = true;
                this.eventQueue.clear();
                return encounterEnded;
            }
            // Labyrinth wipe: log the room as a death with the monster's
            // surviving HP%. enemies is still live here (it is nulled below
            // at the labyrinth reset), so read it before that happens.
            if (this.labyrinth && this.enemies && this.enemies[0]) {
                let mob = this.enemies[0].combatDetails;
                let pct = mob.maxHitpoints > 0 ? (mob.currentHitpoints / mob.maxHitpoints) * 100 : 0;
                this.simResult.addLabRoomOutcome("death", Math.max(0, pct), this.simulationTime, this.labyrinth.enconterStartTime);
            }
        }
    ...
    processRegenTickEvent(event) {
        ...
        // Trial mode: food & drinks are disabled and replaced by a built-in flat
        // bonus HP/MP regen (default +3% each, overridable on the GuildTrial).
        // Added on top of the unit's own per-10s regen ratio.
        let bonusHpRegen = this.guildTrial ? this.guildTrial.bonusHpRegenRatio : 0;
        let bonusMpRegen = this.guildTrial ? this.guildTrial.bonusMpRegenRatio : 0;
    ...
    checkTriggersForUnit(unit, friendlies, enemies) {
        ...
        // Trial mode: no food, no drinks, no coffee. Never consume anything, so
        // no ConsumableTickEvent is ever scheduled and food/drink loadouts are
        // ignored (the built-in bonus regen replaces them; see processRegenTick).
        if (this.guildTrial) {
            return false;
        }
    ...
    processAbilityDamageEffect(source, ability, abilityEffect) {
        ...
        // OFFICIAL_PARRY_GATE — parry model selection.
        //
        // GUILD TRIALS (official rule: "each incoming attack can receive at
        // most 5 parry attempts"): iterate the cast's targets in hit order;
        // each STRUCK target rolls its OWN parry once, success or failure
        // consuming one of the cast's MAX_PARRY_ATTEMPTS attempts (targets
        // without parry consume nothing). A successful parry negates only that
        // target's damage instance — the parrier counters the caster (the
        // `parryTarget` block below, unchanged) — and the cast CONTINUES to
        // the remaining targets; several targets may each parry within the
        // 5-attempt budget, each with its own counter.
        //
        // ZONE / LABYRINTH (legacy, preserved byte-for-byte for parity): one
        // roll per cast by a random parry-capable defender; a success
        // redirects to the caster and BREAKS the whole cast.
        //
        // If in-game observation shows the official rule is universal, delete
        // the legacy branches guarded by `useOfficialParry` here and in
        // processAutoAttackEvent.
        const useOfficialParry = !!this.guildTrial;
        const parryState = { attempts: 0 };
        ...
        for (let target of targets.filter((unit) => unit && unit.combatDetails.currentHitpoints > 0)) {
            let parryTarget = undefined;
            let targetPrepicked = false;

            if (useOfficialParry) {
                // Resolve the ACTUAL struck target first (threat pick for a
                // monster's single-target ability), then that target rolls its
                // own parry — official semantics tie the attempt to the unit
                // actually being hit.
                targets = targets.filter((unit) => unit && !avoidTarget.includes(unit.hrid) && unit.combatDetails.currentHitpoints > 0);
                if (targets.length <= 0) {
                    break;
                }
                if (!source.isPlayer && abilityEffect.targetType == "enemy") {
                    let cumulativeThreat = 0;
                    let cumulativeRanges = [];
                ...
                parryTarget = this.checkParry(targets);
                isSkipParry = true; //  legacy: parry check only once per cast
            }
            ...
            if (parryTarget) {
                ...
                // OFFICIAL_PARRY_GATE: in trial mode a single-target enemy
                // ability already resolved its threat pick above (before the
                // parry roll) — do not pick twice.
                if (!targetPrepicked) {
                    targets = targets.filter((unit) => unit && !avoidTarget.includes(unit.hrid) && unit.combatDetails.currentHitpoints > 0);
            ...
            if (parryTarget && !useOfficialParry) {
                // Legacy: one successful parry breaks the whole cast.
                // Official (trial): only this target's instance was negated —
                // fall through so the cast continues to the remaining targets
                // (single-target "enemy" casts still end via the break below).
                break;
            }
    ...
    processAbilityReviveEffect(source, ability, abilityEffect) {
        ...
        if (reviveTarget) {
            ...
            // Trial: revive is the ONLY way back for a dead player. Clear the
            // "already counted dead" mark so a later re-death is counted again.
            // (The /abilities/revive path does not depend on PlayerRespawnEvent,
            // so dead-stay-dead in trial mode does not break revive.)
            this.trialDeadPlayers?.delete(reviveTarget.hrid);
