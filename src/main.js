...
// MWIX adaptation helpers ----------------------------------------------------
// Read the four labyrinth shop-upgrade levels from the MWIX-added input
// fields. Returns null if the inputs are absent (older HTML in cache), so
// the worker can short-circuit cleanly. Values are clamped to 0..40 to
// mirror the in-game cap.
function readMwixLabUpgradesFromUI() {
    function lv(id) {
        const el = document.getElementById(id);
        if (!el) return null;
        const n = Math.max(0, Math.min(40, Math.floor(Number(el.value) || 0)));
        return n;
    }
    const cd = lv("mwixLabCombatDamageInput");
    if (cd === null) return null; // box not present at all
    return {
        combatDamage: cd,
        attackSpeed:  lv("mwixLabAttackSpeedInput")  || 0,
        castSpeed:    lv("mwixLabCastSpeedInput")    || 0,
        criticalRate: lv("mwixLabCriticalRateInput") || 0,
    };
}
// Read the "labyrinth run" toggle. When enabled, the worker applies the
// lab-shop combat upgrades (extraBuffs). The labyrinth's intrinsic +levels/
// atk/crit bonuses are NOT computed here — they come from the coffee/food
// crates carried into the run (worker assigns them via player.zoneBuffs).
// The old maze-override path was removed: it duplicated the crate buffs.
function readMwixMazeFromUI() {
    const el = document.getElementById("mwixMazeToggle");
    if (!el) return null;
    return { enabled: !!el.checked };
}
...
function showKills(simResult, playerToDisplay) {
    ...
    // MWIX: the visible "Drops" list is now driven by the NO-RNG drop map
    // (expected values per hour). The RNG-rolled list was removed from the
    // UI per user request — but we still iterate the RNG map below so the
    // revenue modal's RNG-side table stays populated.
    ...
    // MWIX: labyrinth-specific stats. Each attempt is one getMonster() call
    // (simResult.labyAttemptCount); an attempt either clears the room
    // (counted in simResult.encounters via addEncounterEnd) or times out
    // after 120s. Completion chance = clears / attempts; timeouts = the
    // remainder, reported per hour like the other rates.
    if (simResult.isLabyrinth) {
        let attempts = simResult.labyAttemptCount || 0;
        let completions = simResult.encounters || 0;
        let timeouts = Math.max(0, attempts - completions);

        let completionChance = attempts > 0
            ? (completions / attempts * 100).toFixed(1) + "%"
            : "—";
        let completionChanceRow = createRow(["col-md-6", "col-md-6 text-end"], ["Completion Chance", completionChance]);
        newChildren.push(completionChanceRow);

        let timeoutsPerHour = hoursSimulated > 0
            ? (timeouts / hoursSimulated).toFixed(1)
            : "0";
        let timeoutsRow = createRow(["col-md-6", "col-md-6 text-end"], ["Timeouts", timeoutsPerHour]);
        newChildren.push(timeoutsRow);
    }
    ...
    for (let [name, dropAmount] of totalDropMap.entries()) {
        // MWIX: dropRow built only for revenue-modal use; NOT pushed to
        // newDropChildren — the visible Drops list now shows no-RNG values.
    ...
    for (let [name, dropAmount] of noRngTotalDropMap.entries()) {
        ...
        // MWIX: no-RNG entries feed the (single, renamed) visible Drops list.
        newDropChildren.push(noRngDropRow);
...
function startSimulation(selectedPlayers) {
    ...
    // MWIX adaptation: fold the labyrinth-shop upgrade levels + the
    // "labyrinth run" toggle into `extra`. The worker builds extraBuffs for
    // the four combat upgrades when the toggle is on. The lab's intrinsic
    // +levels/atk/crit bonuses come from the coffee/food crates, not here.
    extra.mwixLabUpgrades = readMwixLabUpgradesFromUI();
    extra.mwixMaze = readMwixMazeFromUI();
...
document.getElementById("buttonUploadJSONSimulate").addEventListener("click", (event) => {
    ...
    // MWIX adaptation — see the equivalent block in the main simulate path.
    extra.mwixLabUpgrades = readMwixLabUpgradesFromUI();
    extra.mwixMaze = readMwixMazeFromUI();
...
// ---- Iron mode: time-to-acquire values from the local cow webapp ----

const IRON_API_BASE = 'http://127.0.0.1:12345';

function ironModeEnabled() {
    const el = document.getElementById('inputIronMode');
    return !!(el && el.checked);
}

async function fetchIronPrices() {
    const btn = document.querySelector('#buttonGetPrices');
    const select = document.getElementById('selectIronCharacter');
    const character = select && select.value ? select.value : '';
    const url = IRON_API_BASE + '/api/value/market' + (character ? '?character=' + encodeURIComponent(character) : '');
    ...
    try {
        ...
        console.error('Iron mode: failed to fetch time values from cow webapp', error);
        if (btn) btn.style.backgroundColor = 'darkred';
        return;
    }
    ...
    for (const item in itemDetailMap) {
        ...
        // Values are seconds-to-acquire. Vendor trades pay coins, which
        // are worth no time in iron mode.
        window.prices[hrid] = { "ask": v, "bid": v, "vendor": 0 };
    }
    window.prices["/items/coin"] = { "ask": 0, "bid": 0, "vendor": 0 };

    for (const chest of ["/items/small_treasure_chest", "/items/medium_treasure_chest", "/items/large_treasure_chest"]) {
        const drops = openableLootDropMap[chest];
        if (!drops) {
            continue;
        }
        const expected = drops.map((item) => {
            const p = window.prices[item.itemHrid];
            return p && p.ask > 0 ? p.ask * item.dropRate * (item.maxCount + item.minCount) / 2 : 0;
        }).reduce((a, b) => a + b, 0);
        window.prices[chest] = { "ask": expected, "bid": expected, "vendor": 0 };
    }

    if (btn) btn.style.backgroundColor = 'green';
}

async function populateIronCharacters() {
    const select = document.getElementById('selectIronCharacter');
    if (!select) {
        return;
    }
    ...
    try {
        const response = await fetch(IRON_API_BASE + '/api/characters', { mode: 'cors' });
        if (!response.ok) {
            throw new Error('HTTP ' + response.status);
        }
        const payload = await response.json();
        const names = payload['characters'] || [];
        if (names.length === 0) {
            setPlaceholder('(no characters)');
            return;
        }
        select.innerHTML = '';
        ...
        const remembered = localStorage.getItem('ironModeCharacter');
        if (remembered && names.includes(remembered)) {
            select.value = remembered;
        }
    } catch (error) {
        console.log('Iron mode: cow webapp not reachable for character list', error);
        setPlaceholder('(webapp offline)');
    }
}

function initIronModeControls() {
    const toggle = document.getElementById('inputIronMode');
    const select = document.getElementById('selectIronCharacter');
    if (!toggle || !select) {
        return;
    }
    const stored = localStorage.getItem('ironModePrices');
    toggle.checked = stored == null ? true : stored === 'true'; // default ON
    toggle.onchange = () => {
        localStorage.setItem('ironModePrices', String(toggle.checked));
    };
    select.onchange = () => {
        if (select.value) {
            localStorage.setItem('ironModeCharacter', select.value);
        }
    };
    populateIronCharacters();
}

async function fetchPrices() {
    if (ironModeEnabled()) {
        await fetchIronPrices();
        return;
    }
